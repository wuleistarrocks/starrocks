...
//   https://github.com/apache/incubator-doris/blob/master/fe/fe-core/src/main/java/org/apache/doris/analysis/InsertStmt.java
...
public class InsertStmt extends DdlStmt {
    ...
    private void analyzeSubquery(Analyzer analyzer) throws UserException {
         ...
         * eg: origin targetColumns: (A,B,C), shadow column: __starrocks_shadow_B after
         * processing, targetColumns: (A, B, C, __starrocks_shadow_B), and
        ...
        for (Column column : targetTable.getFullSchema()) {
            ...
            if (column.isNameWithPrefix(CreateMaterializedViewStmt.MATERIALIZED_VIEW_NAME_PREFIX)) {
                ...
                if (refColumn == null) {
                    ErrorReport.reportAnalysisException(ErrorCode.ERR_BAD_FIELD_ERROR, column.getName(),
                            targetTable.getName());
                }
            ...
            if (!origColIdxsForExtendCols.isEmpty()) {
                ...
                for (Pair<Integer, Column> entry : origColIdxsForExtendCols) {
                    if (entry.second == null) {
                        ...
                        Expr e = Expr.substituteList(Lists.newArrayList(entry.second.getDefineExpr()), smap, analyzer,
                                false).get(0);
    ...
    private void analyzeRow(Analyzer analyzer, List<Column> targetColumns, List<ArrayList<Expr>> rows,
        ...
        if (!origColIdxsForExtendCols.isEmpty()) {
             ...
             * eg:
             *      the origin row has exprs: (expr1, expr2, expr3), and targetColumns is (A, B, C, __starrocks_shadow_b)
            ...
            for (Pair<Integer, Column> entry : origColIdxsForExtendCols) {
                if (entry == null) {
                    ...
                    extentedRow.add(Expr
                            .substituteList(Lists.newArrayList(entry.second.getDefineExpr()), smap, analyzer, false)
                            .get(0));
                }
            }
    ...
    private Expr checkTypeCompatibility(Column col, Expr expr) throws AnalysisException {
        if (!col.getType().isDecimalOfAnyVersion() && col.getType().matchesType(expr.getType())) {
            return expr;
        }
